!(function($) {
		...
		$.fn.careerTree = function(options){
			...
			function updateBackground()
			{
				...
				callingElement.attr( 'data-active-rows', rowNum );
			}
			...
			function addResults(roles)
			{
				...
				{
					myContainer.prepend("<div class='" + resultGenericClass + " " + resultDescriptionClass + "' data-id='" + (i + 1) + "'><h3>Salary</h3><p>" + roles[i].salary + "</p><h3>Annual Leave</h3><p>" + roles[i].leave + " days</p></div>");
				}
			}
			function addSalaryResults(roles)
			{
				...
				for ( var i = 0; i != roles.length; ++i )
				{
					myContainer.prepend("<div class='" + resultGenericClass + " " + resultSalaryClass + "' data-id='" + (i + 1) + "'><h3>" + roles[i].long_name + "</h3><p><a href='" + roles[i].link + "'>View jobs</a></p></div>");
				}
			}
			function processIndustry(industry)
			{
				...
				var myContainer = callingElement.find("." + resultContainerClass);
				myContainer.prepend("<div class='" + resultGenericClass + " " + resultIndustryClass + " active'><h3>" + industry.long_name + "</h3><p>" + industry.description + "</p></div>");
			}
			...
			function processExperience(exp)
			{
				...
				{
					myContainer.append("<div class='" + yearClass + "'>" + exp[i] + "</div>");
				}
			}
